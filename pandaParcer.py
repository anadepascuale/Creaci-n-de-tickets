...
df = pd.read_csv(
    "Rtas de forms x UA - Derecho.csv",
...
for index, row in df.iterrows():
   # print(index,row)
    print(df['Marca temporal'][2]) 
    
    try: 
        if(pd.isna(df['TICKET ASOCIADO'][index])): 
            # Verifica si la celda está vacía : isna
            jsonticketrow = {'autorespond':"false", 'source': "API", 'name': row["Apellido/s"].upper()+" "+row["Nombre/s"], 'email':row["Dirección de correo electrónico"], "phone":row["Teléfono de contacto "],'subject':str (['Marca temporal'][2])+""+str (row["Marca"])+" "+str(row["Modelo"])+" "+row["Año estimado de compra"], 'message': row["Marca"]+" "+str(row["Modelo"])+" "+row["Año estimado de compra"]+" "+row["¿Cuánta memoria RAM tiene ?"]+"/"+ row["¿De qué tamaño es el disco rígido?"]+" "+str(row["Aclaraciones o Comentarios"])+" webcam: "+str(row["¿Tiene webcam funcionando?"])+" auris: "+str(row["¿Tiene kit auricular/micrófono?"])}
...
df.to_csv("Rtas de forms x UA - Derecho.csv", index=False)