   ...
   # print(index,row)
    ...
    try: 
        if(pd.isna(df['TICKET ASOCIADO'][index])): 
            ...
            jsonticketrow = {'autorespond':"false", 'source': "API", 'name': str(row["Apellido/s"]).upper()+" "+ str(row["Nombre/s"]), 'email':str(row["Dirección de correo electrónico"]), "phone":str(row["Teléfono de contacto "]),'subject':str (row["Marca temporal"][1])+" "+str (row["Marca"])+" "+str(row["Modelo"])+" "+str(row["Año estimado de compra"]), 'message': str(row["Marca"])+" "+str(row["Modelo"])+" "+ str(row["Año estimado de compra"])+" "+ str(row["¿Cuánta memoria RAM tiene ?"])+"/"+ str(row["¿De qué tamaño es el disco rígido?"])+" "+str(row["Aclaraciones o Comentarios"])+" webcam: "+str(row["¿Tiene webcam funcionando?"])+" auris: "+str(row["¿Tiene kit auricular/micrófono?"])}
...
#df.to_csv("Rtas de forms x UA - ESCMB.csv", index=False)